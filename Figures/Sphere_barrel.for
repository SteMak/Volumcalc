      SUBROUTINE CALC_SPHERE_BARREL(MEASURE)
        CHARACTER*80 MEASURE
        CHARACTER*80 A, B, C
        A = '1'
        B = '1'
        C = '1'
        CALL CALC_SPHERE_BARREL_MENU(MEASURE, A, B, C)
      END
      
      SUBROUTINE CALC_SPHERE_BARREL_MENU(MEASURE, A, B, C)
     ...
     +    '(3) Set height ', VC(1:31)
        ...
        CALL WRITE_SPACE(3)
        PRINT '(A, A, E15.8, A, A)', '                  ',
     +    '     VOLUME =', 0.262*CHTOIN(C)*
     +    (2*CHTOIN(B)**2+CHTOIN(A)**2),
