      SUBROUTINE CALC_PARABOLIC_BARREL(MEASURE)
        CHARACTER*80 MEASURE
        CHARACTER*80 A, B, C
        A = '1'
        B = '1'
        C = '1'
        CALL CALC_PARABOLIC_BARREL_MENU(MEASURE, A, B, C)
      END
      
      SUBROUTINE CALC_PARABOLIC_BARREL_MENU(MEASURE, A, B, C)
     ...
     +    '(0) Go to the menu'
        PRINT *, '                       ',
     +    '(1) Set min diameter ', VA(1:31)
        PRINT *, '                       ',
     +    '(2) Set max diameter ', VB(1:31)
        PRINT *, '                       ',
     +    '(3) Set height ', VC(1:31)
        
        CALL WRITE_SPACE(3)
        PRINT '(A, A, E15.8, A, A)', '                  ',
     +    '     VOLUME =', CHTOIN(C)*3.1415926/60*
     +    (8*CHTOIN(B)**2+4*CHTOIN(B)*CHTOIN(A)+3*CHTOIN(A)**2),
     ...
     +    (A, 'Enter the min diameter                  ')
        IF (CH .EQ. '2') CALL SET_PARAM_DIALOG
     +    (B, 'Enter the min diameter                  ')
