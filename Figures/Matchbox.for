      SUBROUTINE CALC_MATCHBOX(MEASURE)
        CHARACTER*80 MEASURE
        CHARACTER*80 A, B, C
        A = '1'
        B = '1'
        C = '1'
        CALL CALC_MATCHBOX_MENU(MEASURE, A, B, C)
      END
      
      SUBROUTINE CALC_MATCHBOX_MENU(MEASURE, A, B, C)
     ...
     +    '(0) Go to the menu'
        PRINT *, '                       ',
     +    '(1) Set 1st side length ', VA(1:31)
        PRINT *, '                       ',
     +    '(2) Set 2nd side length ', VB(1:31)
        PRINT *, '                       ',
     +    '(3) Set 3rd side length ', VC(1:31)
        
        CALL WRITE_SPACE(3)
        PRINT '(A, A, E15.8, A, A)', '                  ',
     +    '     VOLUME =', CHTOIN(A)*CHTOIN(B)*CHTOIN(C),
     ...
     +    (A, 'Enter the first side length             ')
        IF (CH .EQ. '2') CALL SET_PARAM_DIALOG
     +    (B, 'Enter the second side length            ')
        IF (CH .EQ. '3') CALL SET_PARAM_DIALOG
     +    (C, 'Enter the third side length             ')
        IF (.NOT.(CH .EQ. '0'))
     +    CALL CALC_MATCHBOX_MENU(MEASURE, A, B, C)
      END
